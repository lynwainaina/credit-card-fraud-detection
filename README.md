...
Raw Data (creditcard.csv)
         ...
         ▼
┌─────────────────────┐
│  src/data/loader.py │  Load & validate data with custom rules
...
All models trained on an 80/20 stratified split of `features.csv` (**283,726** rows, 40 features after cleaning, engineering + selection). CV = 5-fold StratifiedKFold on the training set.
...
git clone https://github.com/lynwainaina/credit-card-fraud-detection.git
...
python3 -m src.models.run_training
```
...
| `high_signal_magnitude` | Statistical | Same as above but restricted to V14, V17, V12, V10 (highest                                                                                              |
| `pca_extreme_count`     | Statistical | Count of PCA features with `                                                                                                                             |
...
│
├── data/                         # gitignored — never committed due to size of the files
│   ├── creditcard.csv            # Raw dataset from Kaggle
│   ├── cleaned.csv               # After loader + cleaner
│   └── features.csv              # After feature engineering
|   └── sample_features.csv       # Sample dataset that can be commited to Github and for streamlit app run
│
├── models/                       # gitignored — versioned via MLflow
│   ├── model.pkl                 # Different models that have been trained
│   ├── production_model.pkl      # Final model for serving
...
├── requirements_dev.txt          # Pinned dependencies
└── README.md                     # This file
```
